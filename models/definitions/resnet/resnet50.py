...
def convolutional_block(X, f, filters, stage, block, stride=2):
    ...
    batch_norm_base_name = 'bn{}{}_branch'.format(stage, block)

    X_residual = X

    X = base_block(X, f, filters, stage, block)
    ...
    X_residual = BatchNormalization(axis=3, name='{}1'.format(batch_norm_base_name))(X_residual)
...
def base_block(X, f, filters, stage, block):
    ...
    X = BatchNormalization(axis=3, name='{}2a'.format(batch_norm_base_name))(X)
    X = Activation('relu')(X)

    X = Conv2D(filters=F2, kernel_size=(1,1), strides=(1,1), padding='valid', name='{}2b'.format(conv_name_base), kernel_initializer=glorot_uniform(seed=0))(X)
    X = BatchNormalization(axis=3, name='{}2b'.format(batch_norm_base_name))(X)
    X = Activation('relu')(X)
    ...
    X = BatchNormalization(axis=3, name='{}2c'.format(batch_norm_base_name))(X)
